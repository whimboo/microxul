...
if sys.platform.startswith('linux'):
    ...
    def installApp(appLocation, installDir, appName, greDir):
        greDir = os.path.join(greDir, "bin");
...
""";
    ...
    # Implement xulrunnerStubName, installApp and fixLeafName as above.
    raise Exception("This operating system isn't supported for install-app.py yet!");
...
def validateArguments(cmds):
    # application directory / zip verification
    appLocation = resolvePath(cmds.appLocation);

    # target directory
    installDir = resolvePath(cmds.installDir);

    if (os.path.exists(installDir)):
        raise Exception("installDir must not exist: " + cmds.installDir);

    greDir = resolvePath(cmds.greDir);
    ...
    return appLocation, installDir, appName, greDir;
...
def handleCommandLine():
    ...
    parser = argparse.ArgumentParser(
        ...
        usage="""install-app.py appLocation installDir greDir [--appName APPNAME]
           install-app.py -h
           install-app.py --version
    """
    );
    parser.add_argument(
        "appLocation",
    ...
    appLocation, installDir, appName, greDir = validateArguments(cmds);
    installApp(appLocation, installDir, appName, greDir);
    print appName + " application installed to " + installDir;
